...
class SpeculativeGeneralityMethodQuery: Query {
    ...
    var classString: String {
        return "match  (class)-[:CLASS_OWNS_METHOD]->(m:Method)-[:METHOD_OWNS_ARGUMENT]->(p:Argument)-[:IS_OF_TYPE]->(other_class:Class) where not (m)-[:CALLS|USES]->()<-[:CLASS_OWNS_VARIABLE|CLASS_OWNS_METHOD]-(other_class) and not m.data_string contains (\"=\" + p.name) and not m.data_string contains (\"= \" + p.name) and not m.data_string contains (\":\" + p.name) and not m.data_string contains (\": \" + p.name) and not m.data_string contains (\"(\" + p.name + \")\") and not m.data_string contains (\"(\" + p.name + \",\") and not m.data_string contains (\", \" + p.name + \")\") and not m.data_string contains (\", \" + p.name + \",\") and class.is_interface = false return distinct(class.app_key) as app_key, class.name as class_name, count(distinct m) as number_of_smells"
    }
