...
class MessageChainsQuery: Query {
    ...
    var classString: String {
        return "MATCH (c:Class)-[CLASS_OWNS_METHOD]-(m:Method) where m.max_number_of_chaned_message_calls > \(self.veryHighNumberOfChainedMessages) return distinct(m.app_key) as app_key, c.name as class_name, count(distinct m) as number_of_smells"
    }
