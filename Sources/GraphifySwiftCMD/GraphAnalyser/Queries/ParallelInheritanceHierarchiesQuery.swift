...
class ParallelInheritanceHierarchiesQuery: Query {
    ...
    var string: String {
        ...
        match (other_app:App)-[:APP_OWNS_MODULE]->(:Module)-[:MODULE_OWNS_CLASS]->(other_parent:Class)
        where app = other_app
        match path = (class:Class)-[:EXTENDS*]->(parent)
        match other_path = (other_class:Class)-[:EXTENDS*]->(other_parent)
        where parent <> other_parent and length(path) = length(other_path) and length(path) > 0 and class.name starts with substring(other_class.name, 0, \(self.prefixLength)) and parent.name starts with substring(other_parent.name, 0, \(self.prefixLength))
