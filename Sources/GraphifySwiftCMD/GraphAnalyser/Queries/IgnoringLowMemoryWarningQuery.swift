...
class IgnoringLowMemoryWarningQuery: Query {
    ...
    var classString: String {
        return "match (class:Class) where class.name contains 'ViewController' and not (class)-[:CLASS_OWNS_METHOD]->(:Method{name:'didReceiveMemoryWarning()'}) return distinct(class.app_key) as app_key, class.name as class_name"
    }
